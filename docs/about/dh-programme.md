...
2. **Hypotheses frame the questions (S1).** The
   [Guides Hypotheses](https://github.com/sanskrit-lexicon/csl-guides/blob/main/docs/about/guides-hypotheses.md) page states five falsifiable claims
...
3. **Visualizations and data layers show the evidence (S2 + S3).** Three
   visualizations on the [dictionary landscape](https://github.com/sanskrit-lexicon/csl-guides/blob/main/docs/dictionaries/landscape.mdx) and
   [citation sources](https://github.com/sanskrit-lexicon/csl-guides/blob/main/docs/dictionaries/citation-sources.mdx) pages render the
   csl-atlas evidence (novelty, overlap, citations); two data-layer pages —
   [corpus attestation](https://github.com/sanskrit-lexicon/csl-guides/blob/main/docs/dictionaries/corpus-attestation.mdx) and
   [machine morphology](https://github.com/sanskrit-lexicon/csl-guides/blob/main/docs/dictionaries/machine-morphology.mdx) — turn the S5 feeds
...
4. **The ACL lens sits over all of it (S4).** The programme closes by holding the
   ...
   [metric-presentation convention](https://github.com/sanskrit-lexicon/csl-guides/blob/main/docs/about/publications.md) (metric + n + baseline +
   uncertainty + artifact), [data cards](https://github.com/sanskrit-lexicon/csl-guides/blob/main/docs/developers/data-cards.md) for every
   dataset the site ships, a live [shared task](https://github.com/sanskrit-lexicon/csl-guides/blob/main/docs/about/shared-tasks.md) built from GH-1's
   own coverage gap, and a liveness-verified [venue landscape](https://github.com/sanskrit-lexicon/csl-guides/blob/main/docs/about/publications.md)
...
|---|---|---|
| Which-dictionary routing accuracy (GH-1) | 18/18 = 100%, 95% Wilson CI [82.4%, 100%], vs random-4 (25%) and majority-class (5.6%) baselines — **but** 5 of 6 probe scenarios route to dictionaries the quiz never recommends (9 never-targeted golds) | [GH-1](https://github.com/sanskrit-lexicon/csl-guides/blob/main/docs/about/guides-hypotheses.md#gh-1-which-dictionary-routing-accuracy) |
| Deep-page depth follows size, not novelty (GH-2) | at first measurement: ρ(depth, entries) = 0.56 (n = 44, p = 7.7×10⁻⁵); ρ(depth, unique %) = −0.17 (p = 0.26, n.s.) — **re-tested 26-08-2026 after the four thin pages were deepened: ρ(depth, unique %) = +0.26 (p = 0.084); the size-only reading no longer holds** | [GH-2](https://github.com/sanskrit-lexicon/csl-guides/blob/main/docs/about/guides-hypotheses.md#gh-2-deep-page-depth-follows-size-not-novelty) |
| Quiz track teaches word-*finding*, not entry-*reading* (GH-3) | finding modes carry 18–34 items each; reading modes 1–7 (citation-resolution: 2; grammatical labels: 1) | [GH-3](https://github.com/sanskrit-lexicon/csl-guides/blob/main/docs/about/guides-hypotheses.md#gh-3-the-quiz-track-teaches-word-finding-not-entry-reading) |
| Abbreviation-legend exposure (GH-4) | 95.3% of corpus `<ls>` citations (1,187,169 of 1,245,644) fall in legend-documented dictionaries | [GH-4](https://github.com/sanskrit-lexicon/csl-guides/blob/main/docs/about/guides-hypotheses.md#gh-4-abbreviation-legend-exposure) |
| Difficulty-label calibration (GH-5) | instrumented — opt-in, client-only quiz telemetry shipped; no error-rate data yet | [GH-5](https://github.com/sanskrit-lexicon/csl-guides/blob/main/docs/about/guides-hypotheses.md) |
| Dictionary landscape | 43 dictionaries on year × unique-headword-% × size; BHS 57.6% unique, ACC 43.3%, SKD 37.1%; corpus-wide 1,496,157 entries → 410,259 distinct lemmas | [landscape](https://github.com/sanskrit-lexicon/csl-guides/blob/main/docs/dictionaries/landscape.mdx) |
| Headword-overlap cladogram | 41 dictionaries, UPGMA; recovers the known families (PW–PWG, AP–AP90, CAE–CCS, SHS–WIL–YAT, PWKVN–SCH) | [landscape](https://github.com/sanskrit-lexicon/csl-guides/blob/main/docs/dictionaries/landscape.mdx) |
| Citation sources | 828,505 resolved `<ls>` citations → 912 canonical texts across 11 dictionaries (PWG alone: 536,172) | [citation sources](https://github.com/sanskrit-lexicon/csl-guides/blob/main/docs/dictionaries/citation-sources.mdx) |
| Corpus attestation | top 2,000 of 83,277 DCS lemmas (≈1% of MW's 185,803 entries) account for 77.0% of 4,550,704 corpus tokens | [corpus attestation](https://github.com/sanskrit-lexicon/csl-guides/blob/main/docs/dictionaries/corpus-attestation.mdx) |
| Machine-morphology coverage | 25,140 of 185,803 MW entries (13.5%, a lower bound) have a Heritage article; ā- initial leads at 20.7% | [machine morphology](https://github.com/sanskrit-lexicon/csl-guides/blob/main/docs/dictionaries/machine-morphology.mdx) |
| Shared task: which-dictionary routing | dev 18 / test 6 scenarios over all 44 codes; site quiz router scores 100% dev / 0% test (strict) — the coverage gap made into a benchmark | [shared tasks](https://github.com/sanskrit-lexicon/csl-guides/blob/main/docs/about/shared-tasks.md) |
| Data documentation | data cards for all 9 feeds + 6 quiz datasets (176 items), per the Bender & Friedman data-statement convention | [data cards](https://github.com/sanskrit-lexicon/csl-guides/blob/main/docs/developers/data-cards.md) |
| Publishing venues | 6 live ACL-family venues (WSC §23, ISCLS, LaTeCH-CLfL, NLP4DH, WILDRE, LREC) + JOHD, liveness-verified 07-07-2026 | [publications](https://github.com/sanskrit-lexicon/csl-guides/blob/main/docs/about/publications.md) |
...
[vedic-accent feed](https://github.com/sanskrit-lexicon/csl-guides/blob/main/docs/developers/data-cards.md), 26-08-2026); DharmaMitra
link-only (no bulk-download license); Saṃsādhanī validation-only until a LICENSE
appears upstream.

### S1 — Guides Hypotheses (the questions)

[Five falsifiable hypotheses](https://github.com/sanskrit-lexicon/csl-guides/blob/main/docs/about/guides-hypotheses.md) about the site itself, all
metrics computed by
...
the 41-leaf UPGMA cladogram on [landscape](https://github.com/sanskrit-lexicon/csl-guides/blob/main/docs/dictionaries/landscape.mdx), and the
per-dictionary most-quoted-texts explorer on
[citation sources](https://github.com/sanskrit-lexicon/csl-guides/blob/main/docs/dictionaries/citation-sources.mdx). csl-atlas evidence is
vendored by
...
[corpus attestation](https://github.com/sanskrit-lexicon/csl-guides/blob/main/docs/dictionaries/corpus-attestation.mdx) (cumulative coverage,
...
[machine morphology](https://github.com/sanskrit-lexicon/csl-guides/blob/main/docs/dictionaries/machine-morphology.mdx) (Heritage/INRIA as an
...
every dataset the site ships got a [data card](https://github.com/sanskrit-lexicon/csl-guides/blob/main/docs/developers/data-cards.md);
GH-1's coverage gap became a [shared task](https://github.com/sanskrit-lexicon/csl-guides/blob/main/docs/about/shared-tasks.md) with a dev/test split,
a scorer, and a self-reported leaderboard; and
[publications](https://github.com/sanskrit-lexicon/csl-guides/blob/main/docs/about/publications.md) now documents the live venue landscape and the
