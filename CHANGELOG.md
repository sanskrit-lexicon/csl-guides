...
### Added
- **Vedic-accent feed + section** — the corpus-attestation page grows "The same words in the accented Rigveda": [`scripts/build-vedic-accent.mjs`](scripts/build-vedic-accent.mjs) (`npm run build:vedic-accent`, SHA-256 edge contract with `--check` over the upstream bytes) consumes the LANDED VedaWeb 2.0 bulk export in [VisualDCS/non-derived/vedaweb](https://github.com/gasyoun/VisualDCS/tree/main/non-derived/vedaweb) (Casaretto et al. 2025 udātta-marked word-split, CC BY 4.0; offline — never hits vedaweb.uni-koeln.de), joins its lemma keys against the SLP1-keyed DCS top-2000 (845 joined, 45.3% of all DCS tokens; stated lower bound), and [`src/data/vedic-accent.json`](src/data/vedic-accent.json) renders per-lemma RV token counts plus real attested accented forms via SSR-safe `VedicAccent` components. The long-standing "VedaWeb gated on H096" note is retired across `.ai_state.md`, `dh-programme.md` and `NON_COLOGNE_SOURCES.md` §3 (the export landed 08-07-2026, rights confirmed per H359). Data card added; OxAlpha (`opencode/x-preview-f-free`).
- **GH-2 thin-page uplift** — IEG / PGN / ACC / PUI deep pages deepened 434–455 → 918–1,079 words each, every added claim sourced to `csl-orig/v02/{ieg,pgn,acc,pui}` records or the dictionaries' own digitized front matter (ACC's fully transcribed 1891/1962 preface: subject tags an./dh./fr./gr./ny./tantr., catalogue sigla Oxf./Cambr./IO./W./L.; PGN's whole-monograph structure incl. thematic chapters and the Code of Inscriptions; PUI's `<h>` homonym sub-labels). GH-2's prescribed next test executed: `build-hypothesis-metrics.mjs` re-run — ρ(depth, unique %) moved −0.17 → **+0.264** (p=0.084), retiring the size-only reading; before/after kept visible in [guides-hypotheses](docs/about/guides-hypotheses.md). OxAlpha (`opencode/x-preview-f-free`).
- **Ortho-drift search normalization** ([H3340](https://github.com/gasyoun/Uprava/blob/main/handoffs/H3340-OxAlpha_csl-guides_ortho-drift-search-normalization_22.08.26.md)): full-text search queries are expanded through the SanskritSpellCheck reform maps ([`de_reform_map.tsv`](https://github.com/drdhaval2785/SanskritSpellCheck/blob/master/ortho_drift/de_reform_map.tsv) / [`ru_reform_map.tsv`](https://github.com/drdhaval2785/SanskritSpellCheck/blob/master/ortho_drift/ru_reform_map.tsv), vendored under [`vendor/SanskritSpellCheck/ortho_drift/`](vendor/SanskritSpellCheck/ortho_drift/README.md) with a SHA-256 edge contract per SHARED_CODE §24 — `npm run check:ortho-edge`), so typing one spelling era finds gloss text in the other (`Thier` ↔ `Tier`, `августъ` ↔ `август`). Subset policy keeps only unambiguous same-word reform classes (de `1901-th` + `1901-iren`; all ru `1918-*`) — the mined bare-era tail and the `1901-c` class are excluded because they germanize shared English vocabulary ("that→tat", "author→autor"). Implementation: ejected [`src/theme/SearchBar`](src/theme/SearchBar/SearchBar.jsx) + [`src/theme/SearchPage`](src/theme/SearchPage/SearchPage.jsx) wrap their worker calls with [`src/ortho/normalizeQuery.mjs`](src/ortho/normalizeQuery.mjs); proof: `npm run verify:ortho-query -- --bundle build`. OxAlpha (`opencode/x-preview-f-free`).
