...
- **Ortho-drift search normalization** ([H3340](https://github.com/gasyoun/Uprava/blob/main/handoffs/H3340-OxAlpha_csl-guides_ortho-drift-search-normalization_22.08.26.md)): full-text search queries are expanded through the SanskritSpellCheck reform maps ([`de_reform_map.tsv`](https://github.com/drdhaval2785/SanskritSpellCheck/blob/master/ortho_drift/de_reform_map.tsv) / [`ru_reform_map.tsv`](https://github.com/drdhaval2785/SanskritSpellCheck/blob/master/ortho_drift/ru_reform_map.tsv), vendored under [`vendor/SanskritSpellCheck/ortho_drift/`](https://github.com/sanskrit-lexicon/csl-guides/blob/main/vendor/SanskritSpellCheck/ortho_drift/README.md) with a SHA-256 edge contract per SHARED_CODE §24 — `npm run check:ortho-edge`), so typing one spelling era finds gloss text in the other (`Thier` ↔ `Tier`, `августъ` ↔ `август`). Subset policy keeps only unambiguous same-word reform classes (de `1901-th` + `1901-iren`; all ru `1918-*`) — the mined bare-era tail and the `1901-c` class are excluded because they germanize shared English vocabulary ("that→tat", "author→autor"). Implementation: ejected [`src/theme/SearchBar`](https://github.com/sanskrit-lexicon/csl-guides/blob/main/src/theme/SearchBar/SearchBar.jsx) + [`src/theme/SearchPage`](https://github.com/sanskrit-lexicon/csl-guides/blob/main/src/theme/SearchPage/SearchPage.jsx) wrap their worker calls with [`src/ortho/normalizeQuery.mjs`](https://github.com/sanskrit-lexicon/csl-guides/blob/main/src/ortho/normalizeQuery.mjs); proof: `npm run verify:ortho-query -- --bundle build`. OxAlpha (`opencode/x-preview-f-free`).
