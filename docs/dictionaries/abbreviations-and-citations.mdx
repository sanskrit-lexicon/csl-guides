---
id: abbreviations-and-citations
title: Abbreviations & Citations
description: "How to read the abbreviations in CDSL entries (cited texts, authors, grammatical categories) and how to cite the dictionaries correctly."
sidebar_label: Abbreviations & Citations
---

import SiteVersion from '@site/src/components/SiteVersion';

# Abbreviations & Citations

Dictionary entries are dense with **abbreviations** — for cited texts, authors, and
grammatical categories. This page explains how to read them and how to cite CDSL.

## Reading entry abbreviations

Abbreviations in entry bodies are marked up so they can be expanded and (for literary
sources) linked to scans:

| Tag | Role | Example content |
|---|---|---|
| `<ls>` | Literary source citation | a text + locus, e.g. a Veda or epic reference |
| `<ab>` | General abbreviation | `cf.`, `q.v.`, grammatical shorthand |
| `<lex>` | Lexical/grammatical category | gender, part of speech |

Each dictionary has its own abbreviation key, taken from its print front matter. Those
legends are transcribed on the dictionary's [csl-doc](https://github.com/sanskrit-lexicon/csl-doc)
page (`source/dictionaries/{code}.rst` and its `prefaces/`) — the *csl-doc* column in the
[catalog](catalog) links each one.

:::tip See the legends side by side
[**Abbreviations compared**](abbreviations-comparison) lines up every dictionary's
abbreviation legend against the others — how each abbreviates the same cited works and
grammatical terms — and lists each dictionary's full key where it has been transcribed.
:::

Pilot census of OCR pref keys vs body `.txt` (PWG / AP90):
[`scripts/out/`](https://github.com/sanskrit-lexicon/csl-guides/tree/main/scripts/out)
([H1530](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H1530-Sonnet_csl-guides_pref-abbr-body-crosscheck_23.07.26.md)).

## Citing the Cologne Digital Sanskrit Dictionaries

The project's **official citation form** (from the front page):

> **In-text:** (Cologne Digital Sanskrit Dictionaries)
>
> **Bibliographic reference:** Cologne Digital Sanskrit Dictionaries, version <SiteVersion />,
> Cologne University, accessed on [date], https://www.sanskrit-lexicon.uni-koeln.de

When quoting a specific dictionary, also name the **original print work** (author, title,
year — see the [catalog](catalog)).

## Acknowledging data in a website or application

If you reuse the data in a site or app, the project asks for:

> This website / application uses data from Cologne Digital Sanskrit Dictionaries,
> Cologne University, accessed on [date].

## Licensing

The per-dictionary GitHub repositories carry a **CC BY-SA 4.0** `LICENSE` — reuse must
preserve attribution and share-alike. (Confirm terms for any individual dictionary
against its own repo if in doubt.)
