...
### Added
- **Vedic-accent feed + section** — the corpus-attestation page grows "The same words in the accented Rigveda": [`scripts/build-vedic-accent.mjs`](https://github.com/sanskrit-lexicon/csl-guides/blob/main/scripts/build-vedic-accent.mjs) (`npm run build:vedic-accent`, SHA-256 edge contract with `--check` over the upstream bytes) consumes the LANDED VedaWeb 2.0 bulk export in [VisualDCS/non-derived/vedaweb](https://github.com/gasyoun/VisualDCS/tree/main/non-derived/vedaweb) (Casaretto et al. 2025 udātta-marked word-split, CC BY 4.0; offline — never hits vedaweb.uni-koeln.de), joins its lemma keys against the SLP1-keyed DCS top-2000 (845 joined, 45.3% of all DCS tokens; stated lower bound), and [`src/data/vedic-accent.json`](https://github.com/sanskrit-lexicon/csl-guides/blob/main/src/data/vedic-accent.json) renders per-lemma RV token counts plus real attested accented forms via SSR-safe `VedicAccent` components. The long-standing "VedaWeb gated on H096" note is retired across `.ai_state.md`, `dh-programme.md` and `NON_COLOGNE_SOURCES.md` §3 (the export landed 08-07-2026, rights confirmed per H359). Data card added; OxAlpha (`opencode/x-preview-f-free`).
- **GH-2 thin-page uplift** — IEG / PGN / ACC / PUI deep pages deepened 434–455 → 918–1,079 words each, every added claim sourced to `csl-orig/v02/{ieg,pgn,acc,pui}` records or the dictionaries' own digitized front matter (ACC's fully transcribed 1891/1962 preface: subject tags an./dh./fr./gr./ny./tantr., catalogue sigla Oxf./Cambr./IO./W./L.; PGN's whole-monograph structure incl. thematic chapters and the Code of Inscriptions; PUI's `<h>` homonym sub-labels). GH-2's prescribed next test executed: `build-hypothesis-metrics.mjs` re-run — ρ(depth, unique %) moved −0.17 → **+0.264** (p=0.084), retiring the size-only reading; before/after kept visible in [guides-hypotheses](https://github.com/sanskrit-lexicon/csl-guides/blob/main/docs/about/guides-hypotheses.md). OxAlpha (`opencode/x-preview-f-free`).
- **Ortho-drift search normalization** ([H3340](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H3340-OxAlpha_csl-guides_ortho-drift-search-normalization_22.08.26.md)): full-text search queries are expanded through the SanskritSpellCheck reform maps ([`de_reform_map.tsv`](https://github.com/drdhaval2785/SanskritSpellCheck/blob/master/ortho_drift/de_reform_map.tsv) / [`ru_reform_map.tsv`](https://github.com/drdhaval2785/SanskritSpellCheck/blob/master/ortho_drift/ru_reform_map.tsv), vendored under [`vendor/SanskritSpellCheck/ortho_drift/`](https://github.com/sanskrit-lexicon/csl-guides/blob/main/vendor/SanskritSpellCheck/ortho_drift/README.md) with a SHA-256 edge contract per SHARED_CODE §24 — `npm run check:ortho-edge`), so typing one spelling era finds gloss text in the other (`Thier` ↔ `Tier`, `августъ` ↔ `август`). Subset policy keeps only unambiguous same-word reform classes (de `1901-th` + `1901-iren`; all ru `1918-*`) — the mined bare-era tail and the `1901-c` class are excluded because they germanize shared English vocabulary ("that→tat", "author→autor"). Implementation: ejected [`src/theme/SearchBar`](https://github.com/sanskrit-lexicon/csl-guides/blob/main/src/theme/SearchBar/SearchBar.jsx) + [`src/theme/SearchPage`](https://github.com/sanskrit-lexicon/csl-guides/blob/main/src/theme/SearchPage/SearchPage.jsx) wrap their worker calls with [`src/ortho/normalizeQuery.mjs`](https://github.com/sanskrit-lexicon/csl-guides/blob/main/src/ortho/normalizeQuery.mjs); proof: `npm run verify:ortho-query -- --bundle build`. OxAlpha (`opencode/x-preview-f-free`).

## [0.13.1] - 2026-08-06
### Changed
- **UC-2 MW investigation — documented parser delta, not a fix (H2279)** — attempted to unlock MW (struck from UC-2 by H1854 M1) via the existing `pref_abbr_crosscheck.py` → `pref_only_decompose.py` → `pref_legend_emit.py` chain. Finding: MW parses 0 keys because its transcribed front matter ([`MWS/prefaces/mwpref01..29.md`](https://github.com/sanskrit-lexicon/MWS/tree/main/prefaces)) stops at p. xxxii, while MW's actual abbreviation legend — the "List of Works and Authors" — sits at p. xxxiii+, a page that was never OCR'd upstream (confirmed against the live CDSL toctree) and isn't embedded in [`csl-orig/v02/mw/mw.txt`](https://github.com/sanskrit-lexicon/csl-orig/blob/main/v02/mw/mw.txt) either. Documented the delta in [`scripts/pref_abbr_crosscheck.py`](https://github.com/sanskrit-lexicon/csl-guides/blob/main/scripts/pref_abbr_crosscheck.py) header and updated the [UC-2 use-case](https://github.com/sanskrit-lexicon/csl-guides/blob/main/docs/dictionaries/preface-front-matter-enrichment-use-cases.md) + its [metadoc](https://github.com/sanskrit-lexicon/csl-guides/blob/main/docs/dictionaries/preface-front-matter-enrichment-use-cases.meta.md) backlog item 4 accordingly. MW stays excluded from UC-2 pending upstream OCR of p. xxxiii+ — no hand-typed legend was substituted. Tracking: [csl-guides#123](https://github.com/sanskrit-lexicon/csl-guides/issues/123).

## [0.13.0] - 2026-08-06

### Added
- **August 2026 newsletter** ([#162](https://github.com/sanskrit-lexicon/csl-guides/pull/162), [H1853](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H1853-Fable_csl-newsletter_august-2026-cdsl-newsletter-edition-prose_29.07.26.md)): BEN literary-source overhaul, H1523 security sweep, Salt multidict API, preface legend stores, csl-atlas releases, KeySwap. Twin of [csl-newsletter#10](https://github.com/sanskrit-lexicon/csl-newsletter/pull/10) (v0.2.0). Fable 5 (`claude-fable-5`).

## [0.12.0] - 2026-08-04

### Changed
- **Preface-enrichment proposal hostile read ([H1854](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H1854-Fable_csl-guides_preface-enrichment-proposal-hostile-read_29.07.26.md))**: every claim and P1 row of [preface-front-matter-enrichment-use-cases](https://github.com/sanskrit-lexicon/csl-guides/blob/main/docs/dictionaries/preface-front-matter-enrichment-use-cases.md) adversarially verified against committed artifacts — 3 Major / 4 Minor findings recorded in the metadoc; UC-4 confirmed shipped and moved out of P1; UC-2 survives narrowed (MW struck — no committed MW legend artifact); UC-6 survives narrowed (per-dict missing/unused-legend split only; corpus-wide bound already answered by GH-4). Fable 5 (`claude-fable-5`).

## [0.11.0] - 2026-07-31
### Added
- **Preface enrichment P0 ask-batch plan set** (#130).
- **PWG/PW preface OCR+EN sample quality probe (H1559)** (#125).

### Fixed
- **Lychee link-check** series (H1738): exclude private/Uprava blobs, wrong-owner IEG, buttondown/sarit hosts; widen timeouts/retries; exclude DCH 403 + OHD timeout false reds (#143–#149).

### Changed
- **Re-vendor sanskrit-util v0.5.1 (H1969)** (#151).
